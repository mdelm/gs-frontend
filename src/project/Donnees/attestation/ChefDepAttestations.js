...
const ChefDepAttestations = (props) => {
  ...
  const getAttestations = () => {
		axios.get("http://localhost:3000/attestation/all")
			.then(res => {
				setAttestations(res.data.data.filter(att => att.chef_departement === user._id && att.confirmed_by_chef_departement === false));
			});
	}
	const getEtudiants = () => {
	...
	const handleConfirmer = (_id) => {
		axios.put("http://localhost:3000/attestation/confirm/chef", { attestationId: _id })
			.then(res => {
				getAttestations();
			});
	};
...
export default ChefDepAttestations;